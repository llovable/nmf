#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""mean / Ridge / MOCHI-v5 / MOCHI-shared / MIMIR 만 같은 MCAR·MNAR·블록에서 비교."""
...
def main():
    ...
    ap.add_argument("--shared_ckpt", default="/home/dyan/nmf/mochi_code/results/current/gate_shared/shared_best.ckpt")
    ap.add_argument("--out_dir", default="/home/dyan/nmf/mochi_code/results/current/gate_shared")
    ...
    print("loading v5 / MIMIR / shared...")
    mochi2 = load_v5(args.v5_ckpt, dims["rna"], dims["protein"], dims["methyl"], device)
    mimir, mimir_mv = load_mimir(args.mimir_dir, device)
    ckpt = Path(args.shared_ckpt)
    if not ckpt.exists():
        raise FileNotFoundError(f"train_shared.py 먼저 실행: {ckpt}")
    shared = load_shared(ckpt, dims, device)
    shared_tag = f"{getattr(shared, 'fuse_name', 'attn')}-z"
        ...
        for mechanism in ("mcar", "mnar"):
            for rate in RATES:
                for seed in SEEDS:
                    ...
                    rows += score_method("MOCHI-shared", shared_tag, mechanism, split, rate, seed, ytrue,
                                         predict_shared(shared, filled, device, missing=None),
                                         masks, stats)
